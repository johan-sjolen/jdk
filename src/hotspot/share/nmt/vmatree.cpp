...
void VMATree::update_region(TreeNode* n1, TreeNode* n2, const RequestInfo& req, SummaryDiff& diff) {
...
VMATree::SummaryDiff VMATree::register_mapping(position _A, position _B, StateType state,
  ...
  TreeNode* W = rB.start;
  TreeNode* U = rB.end;
  TreeNode nA{_A, stA, 0}; // the node that represents A
  TreeNode nB{_B, stB, 0}; // the node that represents B
  TreeNode* A = &nA;
  TreeNode* B = &nB;
  auto upsert_if= [&](TreeNode* node) {
    if (!node->val().is_noop()) {
      _tree.upsert(node->key(), node->val());
    }
  };
  // update region between n1 and n2
  auto update = [&](TreeNode* n1, TreeNode* n2) {
    update_region(n1, n2, req, diff);
  };
  auto remove_if = [&](TreeNode* node) -> bool{
    if (node->val().is_noop()) {
...
void VMATree::print_on(outputStream* out) {
  visit_in_order([&](TreeNode* current) {
...
VMATree::SummaryDiff VMATree::set_tag(const position start, const size size, const MemTag tag) {
  auto pos = [](TreeNode* n) { return n->key(); };
  position from = start;
